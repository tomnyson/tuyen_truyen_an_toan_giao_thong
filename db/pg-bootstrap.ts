...
// Bảng chủ đề / lĩnh vực nội dung (US-047, DEC-028).
...
// Bảng kho văn bản pháp luật tra cứu (US-049).
const createLegalDocumentsTable = `
CREATE TABLE IF NOT EXISTS legal_documents (
  id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  title text NOT NULL,
  document_number text NOT NULL,
  document_type text DEFAULT 'nghi_dinh' NOT NULL,
  topic text NOT NULL,
  issuing_authority text NOT NULL,
  official_url text NOT NULL,
  summary text DEFAULT '' NOT NULL,
  effectivity_status text DEFAULT 'in_force' NOT NULL,
  status text DEFAULT 'published' NOT NULL,
  ...
  CONSTRAINT legal_documents_document_type_check
    CHECK (document_type IN ('luat', 'nghi_dinh', 'thong_tu', 'quyet_dinh', 'van_ban_hop_nhat', 'khac')),
  CONSTRAINT legal_documents_effectivity_status_check
    CHECK (effectivity_status IN ('in_force', 'expired', 'superseded', 'draft')),
  CONSTRAINT legal_documents_status_check
    CHECK (status IN ('draft', 'published', 'archived')),
  CONSTRAINT legal_documents_link_status_check
    CHECK (link_status IN ('ok', 'broken', 'redirect', 'timeout', 'unchecked')),
  CONSTRAINT legal_documents_url_protocol_check
    CHECK (lower(official_url) LIKE 'https://%' OR lower(official_url) LIKE 'http://%')
)`;

// Bảng tài khoản quản trị & biên tập viên (US-050).
...
CREATE TABLE IF NOT EXISTS admin_accounts (
  ...
  CONSTRAINT admin_accounts_allowed_topics_json_check
    CHECK (
      allowed_topics::jsonb IS NOT NULL
      AND jsonb_typeof(allowed_topics::jsonb) = 'array'
    )
)`;

// Bảng nhật ký kiểm toán hệ thống (Audit Logs) (US-050).
const createSystemAuditLogsTable = `
CREATE TABLE IF NOT EXISTS system_audit_logs (
  id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  actor text NOT NULL,
  actor_role text DEFAULT 'editor' NOT NULL,
...
const indexStatements = [
  ...
  `CREATE INDEX IF NOT EXISTS content_topics_status_order_idx
   ON content_topics (status, display_order)`,
  ...
  `CREATE INDEX IF NOT EXISTS system_audit_logs_actor_idx
   ON system_audit_logs (actor)`,
  `CREATE INDEX IF NOT EXISTS system_audit_logs_target_type_idx
   ON system_audit_logs (target_type)`,
...
export const pgSchemaVersion = "2026-09-13-rbac-and-audit-v1";
...
export const pgBootstrapStatements: readonly string[] = [
  ...
  createReferralAuthoritiesTable,
  createContentTopicsTable,
  createLegalDocumentsTable,
  createAdminAccountsTable,
  createSystemAuditLogsTable,
