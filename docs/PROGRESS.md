...
| Bản nâng cấp 2026-09-13 (Soạn đơn tố giác & CCCD) | 1 | 0 | 0 | 0 |
| **Tổng** | **23** | **18** | **2** | **0** |
...
| US-050 — Phân quyền tài khoản theo chuyên mục & Nhật ký hệ thống | P0 | Done | Full-stack | `db/pg-schema.ts`, `db/pg-bootstrap.ts` (`pgSchemaVersion = 2026-09-13-rbac-and-audit-v1`), `lib/account-store.ts`, `lib/audit-log-store.ts`, `lib/admin-auth.ts`, `app/admin/api/accounts/route.ts`, `app/admin/api/audit-logs/route.ts`, `app/admin/api/content/route.ts`, `app/admin/api/login/route.ts`, `app/admin/api/logout/route.ts`, `app/admin/AccountManager.tsx`, `app/admin/AuditLogManager.tsx`, `app/admin/AdminDashboard.tsx`; `tests/admin-accounts-schema.test.mjs` 2/2, `tests/account-and-audit-store.test.mjs` 3/3, `tests/admin-accounts-api.test.mjs` 2/2, `tests/admin-accounts-ui.test.mjs` 4/4, `tests/rbac-and-audit-e2e.test.mjs` 1/1 pass (DEC-031); checkbox toggle functional state update & onChange binding verified | 2026-09-13 |
| US-051 — Trợ lý AI hỗ trợ soạn đơn tố giác tội phạm & Quét CCCD | P0 | Done | Full-stack | `lib/cccd-parser.ts`, `lib/complaint-form-state.ts`, `lib/docx-generator.ts`, `lib/legal-aid-interview.ts`, `app/api/legal-aid/interview/route.ts`, `components/CccdQrScanner.tsx`, `components/ComplaintDocumentPreview.tsx`, `components/SubmissionGuidanceModal.tsx`, `app/tro-giup-phap-ly/soan-don/page.tsx`, `app/styles/legal-aid.css`, `app/tro-giup-phap-ly/page.tsx`; `tests/cccd-parser.test.mjs` 4/4, `tests/complaint-form-state.test.mjs` 2/2, `tests/docx-generator.test.mjs` 1/1, `tests/legal-aid-interview.test.mjs` 5/5, `tests/cccd-qr-scanner-ui.test.mjs` 2/2, `tests/complaint-document-preview-ui.test.mjs` 3/3, `tests/submission-guidance.test.mjs` 1/1, `tests/soan-don-page.test.mjs` 3/3 pass (21/21), full suite 564/564 pass, `tsc --noEmit` 0 lỗi; Context-aware interview engine phân tích lịch sử hội thoại tự động trích xuất đối tượng, hành vi, số tiền, thời gian; Trực tiếp gõ/sửa vào bản thảo đơn A4 (.inline-doc-input / textarea); In ấn chuẩn Nghị định 30/2020/NĐ-CP giấu toàn bộ website chrome (DEC-032) | 2026-09-13 |
